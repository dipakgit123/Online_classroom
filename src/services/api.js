...
const API_URL = 'http://127.0.0.1:8081/exams'; // Replace with your actual API URL

// Fetch all exams
export const fetchExams = async () => {
  try {
    const response = await axios.get('http://127.0.0.1:8081/exams');
...
export const addOrUpdateExam = async (exams, examId = null) => {
  try {
    if (examId) {
      const response = await axios.put(`${API_URL}/${examId}`, exams);
